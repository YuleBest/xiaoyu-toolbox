...
function performSearch(
  ...
  if (finalResults.length === 0) {
    ...
    if (brandKw) {
      ...
      const brandFiltered = filtered.filter((m) => m.brand === brandKw || m.brand_title === brandKw)
 ...
 * 获取品牌列表统计
 */
export async function getBrandStats(page = 1, limit = 100): Promise<PaginatedResponse<BrandStats>> {
  await loadData()
  const models = allModels || []

  const brandMap = new Map<string, { brand: string; brand_title: string; count: number }>()
  ...
  const allBrands = Array.from(brandMap.values()).sort((a, b) => b.count - a.count)
 ...
 * 搜索机型
 */
export async function searchModels(params: SearchParams): Promise<PaginatedResponse<MobileModel>> {
  await loadData()
  const models = allModels || []
  ...
  // Search
  const { results: searchResultArr, usedQuery, fallbackType } = performSearch(q, filtered)
