...
const downloadStream = (stream: VideoStream | AudioStream, type: 'video' | 'audio') => {
  const ext = type === 'video' ? 'mp4' : 'm4a'
  const qualityDesc = 'qualityDesc' in stream ? stream.qualityDesc : stream.quality
...
const mergeAndDownload = async () => {
  ...
  try {
    ...
    const videoUrl = getDownloadUrl(selectedVideo.value.url, 'temp.mp4', 'video')
    const audioUrl = getDownloadUrl(selectedAudio.value.url, 'temp.m4a', 'audio')
          ...
          <div class="bg-card/30 border border-muted/80 rounded-3xl overflow-hidden">
            <div class="md:flex">
              ...
              <div class="p-5 flex flex-col gap-3 flex-1">
                <h3 class="text-base font-bold text-foreground line-clamp-2 leading-snug">
                  {{ result.title }}
                </h3>
          ...
          >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              ...
              <div>
                ...
                >
                  <option v-for="vs in result.videoStreams" :key="vs.quality" :value="vs">
              ...
              <div>
                ...
                <select
                  ...
                  class="w-full px-3 py-2 bg-background border border-muted rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                >
                  <option v-for="as2 in result.audioStreams" :key="as2.id" :value="as2">
            ...
            <!-- Progress Bar -->
            <div v-if="merging" class="mt-3 h-1.5 bg-muted rounded-full overflow-hidden">
              <div
                ...
                <div class="flex items-center gap-3 min-w-0">
                  ...
                  <div class="min-w-0">
                    <p class="text-sm font-medium truncate">
                      {{ vs.qualityDesc }}
                      <span v-if="vs.superscript" class="text-[10px] text-amber-500 ml-1">{{
                        vs.superscript
                      }}</span>
                    </p>
                    <p class="text-xs text-muted-foreground">
              ...
              >
                <div class="flex items-center gap-3 min-w-0">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-xs text-muted-foreground">
                      {{ as2.codecs }} ·
                      {{ formatSize((as2.bandwidth * (result?.duration || 0)) / 8) }}
                    </p>
                  </div>
                </div>
                <button
