  ...
  <ToolContainer :tool="tool">
    ...
    >
      ...
      <div class="text-center px-6 pointer-events-none z-10">
        <!-- Icon -->
        <div class="mb-6">
          <Zap v-if="gameState === 'waiting'" class="h-20 w-20 text-white/80 mx-auto" />
          <div v-else-if="gameState === 'ready'" class="flex items-center justify-center gap-2">
          ...
          />
          <Clock v-else-if="gameState === 'result'" class="h-20 w-20 text-white/80 mx-auto" />
          <AlertCircle v-else-if="gameState === 'early'" class="h-20 w-20 text-white/80 mx-auto" />
        </div>
        ...
        <div v-if="gameState === 'now'" class="mt-8">
          <span class="text-6xl md:text-7xl font-bold text-white font-mono" style="">
            {{ liveTimer ?? 0 }} ms
          </span>
        </div>
        ...
        <div v-if="gameState === 'result'" class="mt-8 space-y-4">
          <div class="text-6xl md:text-7xl font-black text-white font-mono" style="">
            {{ reactionTime }} ms
          </div>
      ...
      <!-- Bottom Hint -->
      <div v-if="stateConfig.hint" class="absolute bottom-8 text-white/50 text-sm">
        {{ stateConfig.hint }}
      </div>
    </div>
  </ToolContainer>
