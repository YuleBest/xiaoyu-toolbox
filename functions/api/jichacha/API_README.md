...
获取数据库上次更新的时间（从 GitHub 源获取）。

- **URL**: `/time`
- **方法**: `GET`
- **描述**: 返回一个文本字符串，表示更新时间，格式如 `2026-02-19 06:53:15`。
...
3.  **结果聚合**: 响应中会返回当前搜索结果对应各 `dtype` 的数量统计，以及 `ver_name` 的数量统计。
...
```json
{
  ...
  "verNames": [{ "ver_name": "Global", "count": 1 }],
  "results": [
    {
      "id": 1,
      "brand": "apple",
